...
- **Key clinical modules:**
  ...
  - Comprehensive PDF export ("Print Plan" in the plan view header): `buildReadinessPlanHTML(data, plan, sel)` builds a full standalone HTML document ("Your Perioperative Readiness Plan") directly from the in-memory `data` + `plan` objects, then opens it in a new window and calls `window.print()` (browser print-to-PDF). Three sections — About You (all intake echoed verbatim), Your Readiness Plan (every patient card with action steps, targets, Why/Evidence/Citations), and Clinical / Provider Track (disclaimer-led risk summary, alerts, provider cards). All content copied verbatim from data/plan — nothing confabulated.
  - Selective printing: "Print Plan" opens the `PrintCustomizer` modal, which previews the whole document and lets the user click X to delete any part before printing — whole sections, About You groups or individual rows, the risk banner, individual alerts, and individual recommendation cards; plus Card detail chips (action steps / targets / why & evidence) and Everything / Patient handout / Clinical only presets. Removals collapse to an Undo placeholder and never touch the saved plan. Disclaimers are always printed. The preview and the printed HTML share the module-level `PRINT_LBL` / `PRINT_GROUPS` / `printRowValue()` / `printSortRecs()` helpers — **keep them shared, never fork the label or group definitions**, or preview and print will diverge. Omitting `sel` prints everything.
